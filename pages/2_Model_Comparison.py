...
st.success(
    f"✅ Loaded metrics for **{len(all_metrics)}** trained model(s): {', '.join(all_metrics.keys())}"
)
        ...
        with tab:
            ...
            if PLOT_DIR.exists():
                ...
                if shap_plots:
                    ...
                    beeswarm = next(
                        (p for p in shap_plots if "beeswarm" in p.name.lower()), None
                    )
                    importance = next(
                        (p for p in shap_plots if "importance" in p.name.lower()), None
                    )
                    waterfall = next(
                        (p for p in shap_plots if "waterfall" in p.name.lower()), None
                    )
                    ...
                    if waterfall:
                        st.markdown(
                            "##### Waterfall Plot (Local Explanation - Sample #0)"
                        )
